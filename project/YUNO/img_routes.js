var express = require('express');
var router = express.Router();

router.get('/img', function(req, res){
    res.sendFile(__dirname + '/images/1.png');
});

router.get('/assets/background', function(req, res){
    res.sendFile(__dirname + '/assets/background.png');
});

router.get('/assets/board', function(req, res){
    res.sendFile(__dirname + '/assets/board.png');
});
  
router.get('/assets/board2', function(req, res){
    res.sendFile(__dirname + '/assets/board2.png');
});
  
router.get('/assets/board3', function(req, res){
    res.sendFile(__dirname + '/assets/board3.png');
});
  
router.get('/assets/board4', function(req, res){
    res.sendFile(__dirname + '/assets/board4.png');
});
  
router.get('/assets/back_card', function(req, res){
    res.sendFile(__dirname + '/assets/back_card.png');
});
  
router.get('/assets/card_dummy', function(req, res){
    res.sendFile(__dirname + '/assets/card_dummy.png');
});
  
router.get('/assets/yuno_button_on', function(req, res){
    res.sendFile(__dirname + '/assets/yuno_button_on.png');
});
  
router.get('/assets/yuno_button_off', function(req, res){
    res.sendFile(__dirname + '/assets/yuno_button_off.png');
});
  
router.get('/assets/y_card_0', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_0.png');
});
  
router.get('/assets/y_card_1', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_1.png');
});
  
router.get('/assets/y_card_2', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_2.png');
});
  
router.get('/assets/y_card_3', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_3.png');
});
  
router.get('/assets/y_card_4', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_4.png');
});
  
router.get('/assets/y_card_5', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_5.png');
});
  
router.get('/assets/y_card_6', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_6.png');
});
  
router.get('/assets/y_card_7', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_7.png');
});
  
router.get('/assets/y_card_8', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_8.png');
});
  
router.get('/assets/y_card_9', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_9.png');
});
  
router.get('/assets/y_card_arrow', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_arrow.png');
});
  
router.get('/assets/y_card_ban', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_ban.png');
});
  
router.get('/assets/y_card_plus_2', function(req, res){
    res.sendFile(__dirname + '/assets/y_card_plus_2.png');
});
 
router.get('/assets/r_card_0', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_0.png');
});
  
router.get('/assets/r_card_1', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_1.png');
});
  
router.get('/assets/r_card_2', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_2.png');
});
  
router.get('/assets/r_card_3', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_3.png');
});
  
router.get('/assets/r_card_4', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_4.png');
});
  
router.get('/assets/r_card_5', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_5.png');
});
  
router.get('/assets/r_card_6', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_6.png');
});
  
router.get('/assets/r_card_7', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_7.png');
});
  
router.get('/assets/r_card_8', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_8.png');
});
  
router.get('/assets/r_card_9', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_9.png');
});
  
router.get('/assets/r_card_arrow', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_arrow.png');
});
  
router.get('/assets/r_card_ban', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_ban.png');
});
  
router.get('/assets/r_card_plus_2', function(req, res){
    res.sendFile(__dirname + '/assets/r_card_plus_2.png');
});
  
router.get('/assets/g_card_0', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_0.png');
});
  
router.get('/assets/g_card_1', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_1.png');
});
  
router.get('/assets/g_card_2', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_2.png');
});
  
router.get('/assets/g_card_3', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_3.png');
});
  
router.get('/assets/g_card_4', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_4.png');
});
  
router.get('/assets/g_card_5', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_5.png');
});
  
router.get('/assets/g_card_6', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_6.png');
});
  
router.get('/assets/g_card_7', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_7.png');
});
  
router.get('/assets/g_card_8', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_8.png');
});
  
router.get('/assets/g_card_9', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_9.png');
});
  
router.get('/assets/g_card_arrow', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_arrow.png');
});
  
router.get('/assets/g_card_ban', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_ban.png');
});
  
router.get('/assets/g_card_plus_2', function(req, res){
    res.sendFile(__dirname + '/assets/g_card_plus_2.png');
});
  
router.get('/assets/b_card_0', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_0.png');
});
  
router.get('/assets/b_card_1', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_1.png');
});
  
router.get('/assets/b_card_2', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_2.png');
});
  
router.get('/assets/b_card_3', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_3.png');
});
  
router.get('/assets/b_card_4', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_4.png');
});
  
router.get('/assets/b_card_5', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_5.png');
});
  
router.get('/assets/b_card_6', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_6.png');
});
  
router.get('/assets/b_card_7', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_7.png');
});
  
router.get('/assets/b_card_8', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_8.png');
});
  
router.get('/assets/b_card_9', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_9.png');
});
  
router.get('/assets/b_card_arrow', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_arrow.png');
});
  
router.get('/assets/b_card_ban', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_ban.png');
});
  
router.get('/assets/b_card_plus_2', function(req, res){
    res.sendFile(__dirname + '/assets/b_card_plus_2.png');
});
  
router.get('/assets/card_plus_4', function(req, res){
    res.sendFile(__dirname + '/assets/card_plus_4.png');
});
  
router.get('/assets/change_color_card', function(req, res){
    res.sendFile(__dirname + '/assets/change_color_card.png');
});
  
router.get('/assets/num_button_1', function(req, res){
    res.sendFile(__dirname + '/assets/num_button_1.png');
});
  
router.get('/assets/num_button_2', function(req, res){
    res.sendFile(__dirname + '/assets/num_button_2.png');
});
  
router.get('/assets/num_button_3', function(req, res){
    res.sendFile(__dirname + '/assets/num_button_3.png');
});
  
router.get('/assets/num_button_4', function(req, res){
    res.sendFile(__dirname + '/assets/num_button_4.png');
});
  
router.get('/assets/yuno_num_button', function(req, res){
    res.sendFile(__dirname + '/assets/yuno_num_button.png');
});
  
router.get('/assets/other_back_card', function(req, res){
    res.sendFile(__dirname + '/assets/other_back_card.png');
});

router.get('/assets/p_other_back_card', function(req, res){
    res.sendFile(__dirname + '/assets/p_other_back_card.png');
});
  
router.get('/assets/focus_num_button_1', function(req, res){
    res.sendFile(__dirname + '/assets/focus_num_button_1.png');
});
  
router.get('/assets/focus_num_button_2', function(req, res){
    res.sendFile(__dirname + '/assets/focus_num_button_2.png');
});
  
router.get('/assets/focus_num_button_3', function(req, res){
    res.sendFile(__dirname + '/assets/focus_num_button_3.png');
});
  
router.get('/assets/focus_num_button_4', function(req, res){
    res.sendFile(__dirname + '/assets/focus_num_button_4.png');
});
  
router.get('/assets/focus_yuno_num_button', function(req, res){
    res.sendFile(__dirname + '/assets/focus_yuno_num_button.png');
});
  
router.get('/assets/choose_color_board', function(req, res){
    res.sendFile(__dirname + '/assets/choose_color_board.png');
});
  
router.get('/assets/red_color_card', function(req, res){
    res.sendFile(__dirname + '/assets/red_color_card.png');
});
  
router.get('/assets/blue_color_card', function(req, res){
    res.sendFile(__dirname + '/assets/blue_color_card.png');
});
  
router.get('/assets/yellow_color_card', function(req, res){
    res.sendFile(__dirname + '/assets/yellow_color_card.png');
});
  
router.get('/assets/green_color_card', function(req, res){
    res.sendFile(__dirname + '/assets/green_color_card.png');
});
  
router.get('/assets/direction', function(req, res){
    res.sendFile(__dirname + '/assets/direction.png');
});
  
router.get('/assets/direction2', function(req, res){
    res.sendFile(__dirname + '/assets/direction2.png');
});

router.get('/assets/bomb', function(req, res){
    res.sendFile(__dirname + '/assets/bomb.png');
});

router.get('/assets/choose_shield_attack_board', function(req, res){
    res.sendFile(__dirname + '/assets/choose_shield_attack_board.png');
});

router.get('/assets/shield_button_on', function(req, res){
    res.sendFile(__dirname + '/assets/shield_button_on.png');
});

router.get('/assets/shield_button_off', function(req, res){
    res.sendFile(__dirname + '/assets/shield_button_off.png');
});

router.get('/assets/attack_button_on', function(req, res){
    res.sendFile(__dirname + '/assets/attack_button_on.png');
});

router.get('/assets/attack_button_off', function(req, res){
    res.sendFile(__dirname + '/assets/attack_button_off.png');
});

router.get('/assets/u_shield_icon', function(req, res){
    res.sendFile(__dirname + '/assets/u_shield_icon.png');
});

router.get('/assets/o_shield_icon', function(req, res){
    res.sendFile(__dirname + '/assets/o_shield_icon.png');
});

module.exports = router;